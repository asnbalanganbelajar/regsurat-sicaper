...
document.getElementById('formLogin').addEventListener('submit', function(e) {
    ...
    fetch(GAS_API_URL, { method: 'POST', body: JSON.stringify({ action: 'login', username: u, password: p }) })
    ...
    }).catch(err => { showLoading(false); Swal.fire({ icon: 'error', title: 'Error', text: "Koneksi Error: " + err }); });
});
...
function showTab(tabName) {
    ...
    const titles = {
        ...
        'suratKeputusan': ['Pencatatan SK dan SOP', 'Daftar SK dan SOP yang diterbitkan'], 
        'berita_acara': ['Pencatatan Berita Acara', 'Daftar Berita Acara Serah Terima & Pemeriksaan'],
...
function renderDashboard() {
    ...
    document.getElementById('countMasukMonth').innerText = globalData.suratMasuk.filter(row => new Date(row.values[2]).getMonth() === currentMonth && new Date(row.values[2]).getFullYear() === currentYear).length;
    
    document.getElementById('countKeluar').innerText = globalData.suratKeluar.length;
    document.getElementById('countKeluarMonth').innerText = globalData.suratKeluar.filter(row => new Date(row.values[2]).getMonth() === currentMonth && new Date(row.values[2]).getFullYear() === currentYear).length;
    ...
    if(recentMasuk.length === 0) tbodyMasuk.innerHTML = '<tr><td colspan="3" class="text-center text-muted py-3">Belum ada data</td></tr>';
    else recentMasuk.forEach(item => { const r = item.values; const tgl = new Date(r[2]).toLocaleDateString('id-ID', {day:'numeric', month:'short'}); tbodyMasuk.innerHTML += `<tr><td class="text-nowrap">${tgl}</td><td class="fw-bold text-truncate" style="max-width: 100px;">${r[6]}</td> <td class="text-truncate" style="max-width: 150px;">${r[5]}</td> </tr>`; });
    ...
    if(recentKeluar.length === 0) tbodyKeluar.innerHTML = '<tr><td colspan="3" class="text-center text-muted py-3">Belum ada data</td></tr>';
    else recentKeluar.forEach(item => { const r = item.values; const tgl = new Date(r[2]).toLocaleDateString('id-ID', {day:'numeric', month:'short'}); tbodyKeluar.innerHTML += `<tr><td class="text-nowrap">${tgl}</td><td class="fw-bold text-truncate" style="max-width: 100px;">${r[8]}</td> <td class="text-truncate" style="max-width: 150px;">${r[7]}</td> </tr>`; });

    renderChartJS();
}

function renderChartJS() {
    const getStatsByJenis = (dataArray, colIndex) => { const counts = {}; dataArray.forEach(item => { const jenis = item.values[colIndex]; counts[jenis] = (counts[jenis] || 0) + 1; }); return counts; };
    ...
    const createOrUpdateChart = (canvasId, chartInstance, label, dataObj, colorBase) => {
        ...
        if (chartInstance) { chartInstance.data.labels = labels; chartInstance.data.datasets[0].data = data; chartInstance.update(); return chartInstance; } 
        else {
            return new Chart(ctx, { type: 'doughnut', data: { labels: labels, datasets: [{ label: 'Jumlah', data: data, borderWidth: 0, backgroundColor: ['#10b981', '#3b82f6', '#f59e0b', '#ef4444', '#8b5cf6', '#ec4899'] }] }, options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { position: 'right', labels: { boxWidth: 12, font: {size: 10} } } } } });
        }
    };
...
function renderTables() {
    ...
    const createRow = (rowObj, type) => {
        ...
        if (type === 'masuk') {
            ...
            if (displayNoKeluar.includes('/')) { 
                ...
                if (parts.length >= 4) { parts[1] = `<span class="text-primary fw-bold">${parts[1]}</span>`; displayNoKeluar = parts.join('/'); }
                // PERBAIKAN: Format Bold untuk penomoran tanpa kode (3 bagian)
                else if (parts.length === 3) { parts[0] = `<span class="text-primary fw-bold">${parts[0]}</span>`; displayNoKeluar = parts.join('/'); }
            }
    ...
    const createRowBeritaAcara = (rowObj) => {
        ...
        if (displayNo.includes('/')) { 
            ...
            if (parts.length >= 4) { parts[1] = `<span class="text-primary fw-bold">${parts[1]}</span>`; displayNo = parts.join('/'); }
            else if (parts.length === 3) { parts[0] = `<span class="text-primary fw-bold">${parts[0]}</span>`; displayNo = parts.join('/'); }
        }
...
function simpanBon() {
    ...
    const sendRequest = () => {
        ...
        fetch(GAS_API_URL, { method: 'POST', body: JSON.stringify(payload) }).then(res => res.json()).then(resp => { showLoading(false); if(resp.status === 'success') { Swal.fire({ icon: 'success', title: 'Berhasil', text: resp.message, timer: 2000, showConfirmButton: false }); loadData(); } else Swal.fire({ icon: 'error', title: 'Gagal', text: resp.message }); });
    };
...
function hapusBon(rowNumber) {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            showLoading(true); fetch(GAS_API_URL, { method: 'POST', body: JSON.stringify({ action: 'hapusBon', rowNumber: rowNumber, username: currentUser.username }) }).then(res => res.json()).then(resp => { showLoading(false); if(resp.status === 'success') { document.getElementById('searchBon').value = ""; resetPagination('bon'); Swal.fire('Terhapus', resp.message, 'success'); loadData(); } else Swal.fire({ icon: 'error', title: 'Gagal', text: resp.message }); });
        }
    });
}
...
function pushAutoSave(message) { fetch(GAS_API_URL, { method: 'POST', body: JSON.stringify({ action: 'manageJenisSurat', list: globalData.jenisSurat }) }).then(res => res.json()).then(resp => { if (resp.status === 'success') { Toast.fire({ icon: 'success', title: message }); populateDropdown(); } }).catch(err => Toast.fire({ icon: 'error', title: 'Gagal menyimpan perubahan koneksi' })); }
...
function saveUser() {
    ...
    fetch(GAS_API_URL, { method: 'POST', body: JSON.stringify(payload) }).then(res => res.json()).then(resp => { showLoading(false); if (resp.status === 'success') { Swal.fire({ icon: 'success', title: 'Berhasil', text: resp.message, timer: 2000, showConfirmButton: false }); loadUsers(); } else { Swal.fire({ icon: 'error', title: 'Gagal', text: resp.message }); } });
}
...
function deleteUser(username) {
    ...
    }).then((result) => { if (result.isConfirmed) { showLoading(true); fetch(GAS_API_URL, { method: 'POST', body: JSON.stringify({ action: 'manipulateUser', mode: 'hapus', targetUsername: username, adminUsername: currentUser.username }) }).then(res => res.json()).then(resp => { showLoading(false); if(resp.status === 'success') { Swal.fire('Terhapus', resp.message, 'success'); loadUsers(); } else { Swal.fire({ icon: 'error', title: 'Gagal', text: resp.message }); } }); } });
}
...
function openModal(type) {
    ...
    if (type === 'masuk') {
        ...
        title.innerHTML = "<i class='fas fa-gavel me-2'></i>Input SK & SOP"; 
        ...
        jenisInput.innerHTML = '<option value="BA Serah Terima">BA Serah Terima</option><option value="BA Pemeriksaan">BA Pemeriksaan</option><option value="BA Penerimaan">BA Penerimaan</option>';
...
function simpanData() {
    ...
    if (kategori === 'surat_keluar' || kategori === 'surat_keputusan' || kategori === 'berita_acara') { 
        ...
        const isDuplicate = targetDb.some(item => {
            ...
            return ((parseInt(item.values[4]) || 0) === (parseInt(payload.noUrut) || 0) && String(item.values[5]).includes(inputTahun));
        });
        if (isDuplicate) {
            Swal.fire({ title: 'Gagal Simpan!', html: `Nomor urut <b>${payload.noUrut}</b> sudah terdaftar di tahun <b>${inputTahun}</b>.<br>Silakan gunakan nomor lain.`, icon: 'error' }); return; 
        }
    }
...
function kirimKeGAS(payload) {
    fetch(GAS_API_URL, { method: 'POST', body: JSON.stringify(payload) }).then(res => res.json()).then(resp => {
...
function formatAutoPad(input) {
    ...
    else if (input.id === 'nNoUrut' && typeof updatePreviewNpd === "function") updatePreviewNpd(); 
...
function simpanPerjadin() {
    ...
    const sendRequest = () => {
        ...
        .then(res => res.json()).then(resp => { showLoading(false); if(resp.status === 'success') { Swal.fire({ icon: 'success', title: 'Berhasil', text: resp.message, timer: 2000, showConfirmButton: false }); loadData(); } else { Swal.fire({ icon: 'error', title: 'Gagal', text: resp.message }); } });
    };
...
function hapusPerjadin(rowNumber) {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            showLoading(true);
            fetch(GAS_API_URL, { method: 'POST', body: JSON.stringify({ action: 'hapusPerjadin', rowNumber: rowNumber, username: currentUser.username }) })
            .then(res => res.json()).then(resp => { showLoading(false); if(resp.status === 'success') { document.getElementById('searchPerjadin').value = ""; resetPagination('perjadin'); Swal.fire('Terhapus', resp.message, 'success'); loadData(); } else Swal.fire({ icon: 'error', title: 'Gagal', text: resp.message }); });
        }
    });
}
...
function hapusNpd(rowNumber) {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            showLoading(true);
            fetch(GAS_API_URL, { method: 'POST', body: JSON.stringify({ action: 'hapusNpd', rowNumber: rowNumber, username: currentUser.username }) }).then(res => res.json()).then(resp => { showLoading(false); if(resp.status === 'success') { document.getElementById('searchNpd').value = ""; resetPagination('npd'); Swal.fire('Terhapus', resp.message, 'success'); loadData(); } else Swal.fire({ icon: 'error', title: 'Gagal', text: resp.message }); });
        }
    });
}
...
function hapusPesanan(rowNumber) {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            showLoading(true); fetch(GAS_API_URL, { method: 'POST', body: JSON.stringify({ action: 'hapusPesanan', rowNumber: rowNumber, username: currentUser.username }) }).then(res => res.json()).then(resp => { showLoading(false); if(resp.status === 'success') { document.getElementById('searchPesanan').value = ""; resetPagination('pesanan'); Swal.fire('Terhapus', resp.message, 'success'); loadData(); } else Swal.fire({ icon: 'error', title: 'Gagal', text: resp.message }); });
        }
    });
}
...
function processDownload() {
    ...
    fetch(GAS_API_URL, { method: 'POST', body: JSON.stringify({ action: 'downloadRekap', kategori: kategori, startDate: start, endDate: end, format: format }) })
