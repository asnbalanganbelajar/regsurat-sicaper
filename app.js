...
function renderTables() {
    // Cek tab mana yang sedang aktif di layar
    ...
    // Filter Masuk (Hanya dieksekusi jika tab Masuk aktif atau Dashboard aktif)
    ...
    if (isTabMasukActive) {
        ...
        if (termMasuk) {
            filteredMasuk = globalData.suratMasuk.filter(row => {
                return ((row.values[3] || "") + " " + (row.values[4] || "") + " " + (row.values[5] || "") + " " + (row.values[6] || "") + " " + (row.values[7] || "")).toLowerCase().includes(termMasuk);
            });
        }
    }
    ...
    if (isTabKeluarActive) {
        const termKeluar = document.getElementById('searchKeluar') ? document.getElementById('searchKeluar').value.toLowerCase() : '';
        if (termKeluar) {
            filteredKeluar = globalData.suratKeluar.filter(row => {
                return ((row.values[5] || "") + " " + (row.values[6] || "") + " " + (row.values[7] || "") + " " + (row.values[8] || "") + " " + (row.values[9] || "")).toLowerCase().includes(termKeluar);
            });
        }
    }
    ...
    if (isTabKeputusanActive) {
        const termKeputusan = document.getElementById('searchKeputusan') ? document.getElementById('searchKeputusan').value.toLowerCase() : '';
        if (termKeputusan) {
            filteredKeputusan = globalData.suratKeputusan.filter(row => {
                return ((row.values[5] || "") + " " + (row.values[6] || "") + " " + (row.values[7] || "") + " " + (row.values[8] || "") + " " + (row.values[9] || "")).toLowerCase().includes(termKeputusan);
            });
        }
    }
    ...
    if (isTabPerjadinActive) {
        ...
        if (termPerjadin) {
            filteredPerjadin = globalData.perjadin.filter(row => {
                ...
                return ((vals[5] || "") + " " + (vals[6] || "") + " " + (vals[7] || "") + " " + (vals[8] || "") + " " + personilNames).toLowerCase().includes(termPerjadin);
            });
        }
    }
    ...
    if (isTabNpdActive) {
        ...
        if (termNpd) {
            filteredNpd = globalData.npd.filter(row => {
                return ((row.values[4] || "") + " " + (row.values[5] || "") + " " + (row.values[6] || "")).toLowerCase().includes(termNpd);
            });
        }
    }
    ...
    if (paginatedMasuk.length === 0) {
        ...
        // OPTIMASI: Render 1 kali langsung
        tbodyMasuk.innerHTML = paginatedMasuk.map(rowObj => createRow(rowObj, 'masuk')).join('');
    }
    ...
    if (paginatedKeluar.length === 0) {
        ...
        // OPTIMASI: Render 1 kali langsung
        tbodyKeluar.innerHTML = paginatedKeluar.map(rowObj => createRow(rowObj, 'keluar')).join('');
    }
    ...
    if(tbodyPerjadin) {
        ...
        if (paginatedPerjadin.length === 0) {
            ...
            // OPTIMASI: Render 1 kali langsung
            tbodyPerjadin.innerHTML = paginatedPerjadin.map(rowObj => createRowPerjadin(rowObj)).join('');
        }
    ...
    if(tbodyNpd && mobileNpd) {
        if (paginatedNpd.length === 0) {
            ...
            // OPTIMASI: Render 1 kali langsung untuk Desktop dan Mobile
            tbodyNpd.innerHTML = paginatedNpd.map(rowObj => createRowNpd(rowObj)).join('');
            mobileNpd.innerHTML = paginatedNpd.map(rowObj => createCardNpd(rowObj)).join('');
        }
    ...
    if (tbodyKeputusan) {
        ...
        if (paginatedKeputusan.length === 0) {
            ...
            // OPTIMASI: Pindahkan template string yang panjang ke fungsi helper terpisah, 
            // ATAU cukup gunakan map join seperti ini:
            tbodyKeputusan.innerHTML = paginatedKeputusan.map(rowObj => {
                const row = rowObj.values;
                const realRow = rowObj.rowNumber;
                let btnFile = row[11] ? `<button class="btn-circle btn-circle-primary" onclick="previewFile('${row[11]}')" title="Pratinjau File"><i class="fas fa-eye"></i></button>` : `<button class="btn-circle btn-circle-disabled" disabled><i class="fas fa-eye-slash"></i></button>`;
                let tgl = new Date(row[2]).toLocaleDateString('id-ID', { timeZone: 'Asia/Makassar', day: 'numeric', month: 'short', year: '2-digit' });
                const valKet = row[10];
                const ketHtml = valKet ? `<div class="small text-danger fst-italic text-wrap" style="line-height:1.2;"><i class="fas fa-info-circle me-1"></i>${valKet}</div>` : ""; 
                let displayNoKeputusan = row[5] || "";
                if (displayNoKeputusan.includes('/')) {
                    let parts = displayNoKeputusan.split('/');
                    if (parts.length > 1) { parts[1] = `<span class="text-primary fw-bold">${parts[1]}</span>`; displayNoKeputusan = parts.join('/'); }
                }
                return `<tr>
                    <td style="vertical-align: top; padding-top: 12px;"><div class="fw-bold text-dark small">${displayNoKeputusan}</div><small class="text-muted" style="font-size: 0.75rem;"><i class="far fa-calendar-alt me-1"></i> ${tgl}</small></td>
                    <td style="vertical-align: top; padding-top: 12px;"><span class="badge badge-keputusan mb-1">${row[6]}</span><div class="small text-dark text-wrap">${row[7]}</div></td>
                    ...
                    <td class="text-center" style="vertical-align: top; padding-top: 12px;"><div class="d-flex justify-content-center gap-2">${btnFile}
                        <button class="btn-circle btn-circle-warning" onclick="editData('keputusan', ${realRow})"><i class="fas fa-pen"></i></button>
                        <button class="btn-circle btn-circle-danger" onclick="hapusData('keputusan', ${realRow})"><i class="fas fa-trash-alt"></i></button></div>
                    </td></tr>`;
            }).join('');
        }
